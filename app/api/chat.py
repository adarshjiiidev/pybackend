...
async def _run_ai_workflow(
    ...
    try:
        ...
        # Performance Optimization: Reuse the pre-compiled global agent graph
...
async def send_message(
    ...
    try:
        ...
        # Performance Optimization: Fetch only the last 21 messages from MongoDB
        # instead of the entire history to reduce database I/O and memory overhead.
        # We fetch 21 to get the current message (just inserted) + 20 previous messages.
        messages = await Message.find(
            Message.conversation_id == conversation.conversation_id
        ).sort("-created_at").limit(21).to_list()

        # Sort back to chronological order for the agent workflow
        messages.reverse()
        ...
        raw_history = [
            ...
            for msg in messages[:-1]  # Exclude the current user message (last in chronological order)
        ]
